...
| 80 | **The header logo did not go home** | ✅ Reported by the user; I had missed it in the audit above. The logo linked to `/${currentForo}`, so on any foro other than foroscomun the header's only "home" affordance just reloaded the page you were already on. Legacy binds `.js-home` to `mainView.goToHome()` (`mainView.js:104-107`) → route `/` + `scrollTop(0)`. Also corrected the title link, which took its target from the `:foro` URL param rather than the head's `Name` — legacy's `data-link` reads `Name`, so on a ciudadanos wall (route `/ciudadanos/:id`, no `:foro` param) the branch's title pointed at foroscomun. 3 regression tests in `Header.test.js`, verified failing without the fix. |
