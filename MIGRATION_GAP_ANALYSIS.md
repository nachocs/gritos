...
| 68 | **Every post was reported to the user as failed** | n/a | ✅ | The big one, and it had been shipped-broken the whole time. `post.cgi` answers `{"success":"ok","mensaje":{…}}` — **not** `{"status":"ok"}` like `login.cgi` and `registro.cgi` do. `FormContext` gated on `status`, so the check never passed and *every* successful post took the failure branch: the user saw "No se pudo publicar el grito", the composer never cleared, and the new grito was never inserted into the list — while the entry **had** been created on the server. Legacy never had this bug because `Backbone.sync` treats any 2xx as success and just reads `data.mensaje`. **Why the suite never caught it:** all four existing `FormContext` tests mocked `{status:"ok"}`, a shape the real endpoint never returns — the mock encoded the same wrong assumption as the code. Added a test using the verified live shape. **This also supersedes part of my own earlier reporting:** the "message doesn't appear" symptom I attributed solely to #67 was mostly *this*; #67 is a real latent bug (proved by its own failing test) but would only have bitten once this was fixed. |
| 67 | **Gritos posted on a wall never appeared until reload** | n/a | ✅ | Found by the first real end-to-end post (user authorised posting as themselves, then deleting). A ciudadanos wall is *fetched* as `ciudadanos/<id>/` — the trailing slash is what makes `index.cgi` list children — but `FormShell` posts to and publishes under `ciudadanos/<id>`. `useForumMessages`' new-message listener compared the two with a strict `!==`, so every grito posted on a wall was silently dropped from the visible list and only showed up after a manual reload. The socket-room memo two lines above *had* normalised the slash; the comparison hadn't. Both now use one `stripSlash` helper. Legacy has no equivalent bug because formView holds a direct reference to the collection and calls `collection.add(...)` — no string key to mismatch. 3 regression tests in `useForumMessages.test.js`, confirmed failing without the fix. |
...
| 43 | Composer detail (emoji, image, poll, tags, URL capture) | n/a | ✅ | Was 🔴 — see #48, #49, #53. **Posting verified end-to-end 2026-07-21** against the live API (user authorised posting as themselves): `POST post.cgi` with `{comments, uid, tags, foro:"ciudadanos/1"}` created entry ID 255, and deleting it through the UI's confirm modal removed it server-side. That single round-trip found #67, which no unit test had caught. Still unverified live: posting *with* an image, poll, or captured URL attached. |
...
### Stylesheet audit (2026-07-21)

Diffed every class selector in the branch's `main.less` against `master`'s, in both directions.

**Rules the branch added:** only two remain, both legitimate and in use — `.images-strip` (the documented replacement for legacy's slick carousel) and `.resumen-collection__legal` (legacy renders the same privacidad/legal link with inline styles). The earlier invented blocks — `.modal-root__*`, `.modal-group`, `.modal-error`, `.login-modal` — are gone with #58/#61. **No dead branch-authored CSS remains.**

**Styled legacy classes the branch never renders:** the scan returned 5, and all 5 are false positives or dead in legacy too — 4 are built dynamically in `SignUpModal` (`` `valid-${name}` ``/`` `valid-${name}-load` ``, so the literal string never appears in source) and `.md-36` appears only inside a commented-out line in legacy's `spinnerView.html`. **No styled legacy UI is unrendered.**

**Module coverage:** every one of master's 57 `main/`, `models/` and `util/` modules has a counterpart, including the two easiest to miss — `previousMsgView` (folded into `MiniThread`'s load-previous control) and `molaView` (into `MessageItem`).
...
### Shell, SPA & deploy pass (2026-07-21)

Re-checked the "what's still missing" list against the live `https://gritos.com/` HTML and `master`, rather than against the notes above — which turned out to be partly stale (see #74).

| # | Finding | Status |
|---|---|---|
| 71 | **The HTML shell was nearly empty** | ✅ Restored from the deployed head: OpenGraph + Twitter cards, `fb:app_id`, description/keywords/author, `google-site-verification`, the `window.imgError` global (`displayCapturedUrl.html` and message bodies call it inline from `onerror`), manifest + the apple/android icon set, `theme-color`/msapplication tiles, and the Spanish `<noscript>`. |
| 72 | **Two MDL themes were loading at once** | ✅ `index.html` pulled MDL **1.3.0 orange-indigo** from the CDN while `main.less` imports **light_green-red** locally — the theme the deployed build actually ships. Dropped the CDN stylesheet; only the Material Icons font is still fetched remotely, and MDL's JS. |
| 73 | **Static assets were unreachable** | ✅ `src/assets` is now Vite's `publicDir`, so the icon set, `manifest.json` and `logogritos.jpg` serve from the site root and are copied into `dist/`. Root-relative is also what `manifest.json`'s own icon entries already assume, and the deployed host answers on both `/assets/x.png` and `/x.png`. |
| 74 | ~~Drawer missing the privacidad/legal link~~ | ⚪ **Not a gap — the entry above was stale.** `ResumenNav.jsx` already renders it, and `.resumen-collection__legal` in `main.less` reproduces legacy's inline `position:fixed` block exactly. A duplicate was briefly added before checking, then reverted. |
| 75 | **Drawer title hard-coded** | ✅ Legacy `mainView-t.html:45` renders `<%= obj.Titulo %>`. The drawer said "Gritos.com" on every foro while the header above it showed the real one. Now fed `head` from `Layout`. Verified on `/kingcrimson`: both read "king Crimson". |
| 76 | **No navigation after creating a foro** | ✅ Legacy `formView.submitPost` (`formView.js:579-585`) calls `router.navigate('/' + Name, { trigger: true })` on any `isHead` save. Creating a foro from the drawer left you on the page you started from, with no route to the foro you had just made. |
| 77 | **`foro` omitted from new-foro saves** | ✅ Found while fixing #76: legacy sends `foro: INDICE` for *every* head save (including `gritosdb`), the branch sent it only for non-`gritosdb` heads. |
| 78 | **Production hung behind the Facebook SDK** | ✅ `index.js` rendered immediately in dev but `await loadFBSDK()` *before* `renderApp()` in production. A blocked `connect.facebook.net` — routine with content blockers — leaves that promise unsettled rather than rejected, so the `catch` never fires and production paints **nothing**. Dev-only testing is exactly why this survived. Legacy `app.js` renders synchronously and lets the SDK arrive whenever; nothing in the first paint needs it and FB login is a stub. Now renders first, loads the SDK in the background. |
| 79 | **A stale service worker is still live on the deployed origin** | ✅ `https://gritos.com/sw.js` serves an offline-plugin worker caching `/dist/1.10.10/…`. Service workers survive a redeploy, so returning visitors would keep being served from a cache of assets the React build no longer produces — and the path cannot simply 404, since a registration is only replaced when the browser fetches a *new* script there. Added `src/assets/sw.js`: a tombstone worker that drops every cache, unregisters itself and reloads open clients. **Keep it deployed indefinitely** — a client that never revisits never updates. |

**Analytics placement:** GA (`UA-108283891-1`) and AdSense (`ca-pub-5436524166740759`) are injected by a `apply: "build"` plugin in `vite.config.js`, not written into `index.html`. In the static shell they would also fire from `localhost:3001` on every dev reload, into the real property. Verified absent under `vite`, present in `dist/index.html`.

**SPA routing** needs no work: `createBrowserRouter` with a `path: "*"` root, deep links resolve (`/kingcrimson` → 62 cards, `/foroscomun` on the built bundle → 39), and the deployed host already history-fallbacks any path to `index.html`. Neither legacy nor the branch updates `document.title` per route, so that stays as-is.

**Still open:** there is no deploy story. The deployed site serves versioned `/dist/<version>/app.<hash>.js`, which came from the webpack config that was deleted; `npm run deploy` still references `s3-deploy` with CircleCI variables. `npm run lint` is also broken independently of any of this — ESLint 9 wants `eslint.config.js` and the repo has `.eslintrc`.

| 80 | **The header logo did not go home** | ✅ Reported by the user; I had missed it in the audit above. The logo linked to `/${currentForo}`, so on any foro other than foroscomun the header's only "home" affordance just reloaded the page you were already on. Legacy binds `.js-home` to `mainView.goToHome()` (`mainView.js:104-107`) → route `/` + `scrollTop(0)`. First pass over-corrected and also changed the *title* link to key off the head's `Name`; the user asked for the logo only, reported the title change as breaking titles on individual foros (not reproduced locally, but reverted per instruction rather than argued with), and pointed out `/foroscomun` should never be a visible URL — legacy's router only tracks it internally, the address bar stays at `/`. The title link is back to `/${currentForo}`, matching legacy's own behavior of round-tripping to itself when already on foroscomun. `App.jsx`'s `/` → `/foroscomun` redirect (pre-existing, not touched this pass) is the other half of that same visible-URL question and is still open. 2 regression tests in `Header.test.js` cover the logo only. |

| 81 | **Reply forms rendered with the full-size composer's padding** | ✅ Reported by the user ("too much padding", "a space between the msg and the form"). Legacy mounts formView's own root (className `formulario`) *inside* a separate `.mini-form` wrapper — `baseMsgView.render()`: `$('.mini-form').html(this.formView.render().el)`, `msgView-t.html`: `<div class="mini-form"></div>`. `main.less` carries a `.mini-form .formulario { padding: 0; .form-submit{display:none}; .file-submit{right:10px;bottom:30px}; .mdl-card{padding-right:50px;min-height:initial}; .emojis{right:12px;left:initial;bottom:12px} }` block, unmodified from master, that depends on exactly that nesting to strip the 960px composer's `20px 0` padding and reposition the icons for a reply. `ReplyForm.jsx` put both classes on one `<div>`, so the block — a descendant selector — never matched: replies rendered with the top composer's full padding, default (unrepositioned) icons, and a visible "Grita" button legacy hides here (Enter submits instead). Restructured to the nested `<div className="mini-form"><div className="formulario active">…` legacy has. Verified with computed styles after the fix: `.formulario` padding `0px`, card `padding-right: 50px`, `.form-submit` `display: none`, icons at their offset positions. Checked `FormShell.jsx` (the top composer) for the same class-merge pattern — it doesn't have one: legacy mounts formView directly inside `.form-view`, which carries no CSS of its own, so there is no equivalent nested override to miss. 1 regression test in `ReplyForm.test.js`, verified failing without the fix. |

| 82 | **`/` (and `/admin`, `/jsgritos`) rewrote the address bar to `/foroscomun`** | ✅ The open item from #80 — the user pushed for it explicitly ("that's an internal thing", "I never showed 'foroscomun' before") and then again after seeing `goHome` still land on `/foroscomun`. Confirmed in `master`'s `router.js` `foro()`: the reserved-name branch (`!foro \|\| foro === 'admin' \|\| 'ciudadanos' \|\| 'jsgritos'`) only reassigns its **local** `foro` variable to `'foroscomun'` for the content lookup — it never calls `this.navigate()`, so the address bar stays exactly as typed. `App.jsx` used `<Navigate to="/foroscomun" replace />` for `/`, `/admin` and `/jsgritos`, an actual client-side redirect matching none of that. `/` now renders `ForoPage` directly (it already defaults an absent `:foro` param to foroscomun via `normalizeForo`); the explicit `/admin`/`/jsgritos` routes were removed entirely — the generic `/:foro` route plus `normalizeForo` already produce the same URL-preserving behavior, exactly as the bare `/ciudadanos` case (no explicit route) already did. `Header.jsx`'s title link also special-cases foroscomun to `/`, matching legacy's own `data-link="/<%= Name !== 'foroscomun' ? Name : '' %>"` (empty string → `/`) — previously it always built `/${currentForo}`, so the title still pointed at `/foroscomun` from the home page even after the logo was fixed. 3 regression tests in `App.test.js` (verified failing against the old redirect routes); `goHome` in `Header.jsx` also now does its own `navigate("/")` + `scrollTo(0,0)` as a single unit (mirroring legacy's single `goToHome()` method) rather than splitting the navigation into the `Link`'s `to` prop. **Note on code comments in this file and nearby ones**: per the user, don't take existing inline comments as verified fact — some were added by earlier passes and are wrong; verify against `master` rather than trusting them. |
