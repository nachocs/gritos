...
Still open for Phase 5: items 26 (FB login), 28 (avisos banner), 29 (dreamys persistence) — not started.
...
| Login: alias/password dropdown form, error display | ✅ (moved into `LoginModal`) |
...
| 42 | Modals (login, signup, dreamys, edit, confirm) | 🟡 | 🟡 | Edit modal's composer fixed (#57). **Deliberate deviation to confirm:** deployed's logged-out login is a **header dropdown** (`ul.login-menu` holding the alias/password form, the Sign Up button and the FB button), not a modal — the branch opens `LoginModal` instead. Part 1 already records this as an intentional move ("moved into `LoginModal`"), so it's a design call, not a regression — flagging it here so it's a conscious one. Legacy's logged-in menu also carries `mdl-card mdl-shadow--4dp short`, which the branch's `.login-menu logged-in` omits. Signup/dreamys/confirm modals not yet audited. |
| 56 | **Invented class names with no CSS** (method) | — | — | Three separate visual bugs this phase (#48c, #55, #57) had the same shape: markup invented a class (`.form-toolbar`, `.upload`, `.emoji-pick`) that **matches no rule in main.less**, so the element silently fell back to browser defaults. Comparing computed styles of *containers* kept passing while the leaf elements were wrong. Added a scan that diffs every `className` used in `react-app/**/*.jsx` against the class selectors in `main.less`; it surfaced ~56 candidates and found #57 immediately. Worth re-running before declaring Phase 7 done — remaining flagged names are mostly harmless JS hooks (`js-edit`, `js-logout`) or React-only wrappers, but each is worth a glance. |
