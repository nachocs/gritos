...
export default ViewBase.extend({
  ...
  validatePassword(e) {
    ...
    if (password && password.length < 8) {
      this.$(".error-password")
        .html("password de 8 characteres al menos, porfa")
        .addClass("active");
    } else if (password) {
  ...
  checkValue(indice, value) {
    ...
    const gets$ = ajax(
      endpoints.apiUrl + "check.cgi?indice=" + indice + "&value=" + value,
    ).pipe(map((e) => e.response));

    this.checkValueSubscription[indice] = gets$.subscribe((data) => {
      ...
      if (data.status === "disponible") {
        ...
        this.$(".error-" + indice)
          .html("El " + indice + " ya est&aacute; pillao")
          .addClass("active");
